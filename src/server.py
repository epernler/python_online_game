...
HOST = socket.gethostbyname(
    socket.gethostname())  # set the host of server to own local IP, should be changed to public IP when 2 players
ADDRESS = (HOST, PORT)
        ...
        try:  # Try undviker att allt kraschar ifall ett meddelande inte går att ta emot / skickas
            ...
            if space[other_p - 1]:  # Ifall den andra spelaren har klickat space...
                c.sendall(str.encode(
                    "SPACE"))  # Så skickar vi "SPACE" till denna klient, istället för andra spelarens position
                ...
                if not data:
                    ...
                    if data == "SPACE":  # Ifall klienten klickat space
                        ...
                        data = read_pos(
                            data)  # Annars, om klienten inte klickat space så läser vi in klientens position
