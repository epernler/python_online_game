...
def connect_client(c, a, p):
    ...
    while connection:
        ...
        if msg_len:                                         # avoid error since theres a connection message that cant convert to int
            ...
            # har tagit emot ett meddelande från klient, nu avgöra vad för meddelande och ge rätt svar till båda klienterna
...
def init_server():
    ...
    while True:
        ...
        thread = threading.Thread(target=connect_client, args=(c, a, threading.active_count()))
        thread.start()
